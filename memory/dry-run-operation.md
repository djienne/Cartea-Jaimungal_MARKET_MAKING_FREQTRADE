...
Gate-battery & MM_ADV: the disabled smoke runs its gate container under the production name `MM_ADV`, so a `--include-runtime` battery replaces the production bot while it runs. The runner now restores it automatically at the end (`production_restore` in the payload; `--no-restore-production` opts out). If the restore ever fails (WARNING in the runner output): `docker rm -f MM_ADV` then `docker compose up -d freqtrade`, and confirm `Parameter source: redis` + `Trading symbol: ETH` in the logs. Battery profiles: default = fast (~2.5 min, no smokes); `--include-runtime` = full (~17 min); `--include-runtime --reuse-smoke-artifacts` = rerun everything cheap against the last run's smoke artifacts (≤6 h) in ~4 min. Always pass `--json-output docs/last_safety_gates.json` on runtime batteries or the audit evidence is not persisted.
