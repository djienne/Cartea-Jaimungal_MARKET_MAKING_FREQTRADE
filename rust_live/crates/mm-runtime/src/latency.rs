...
impl LatencyKind {
    const ALL: [Self; 21] = [
        ...
        Self::ExecutionQueueWait,
        Self::ActionPreparation,
        Self::Signing,
        Self::SocketWrite,
        Self::DecisionToSocketWrite,
    ...
    const fn name(self) -> &'static str {
        match self {
            ...
            Self::ActionPreparation => "action_preparation",
            Self::Signing => "signing",
            Self::SocketWrite => "socket_write",
            Self::DecisionToSocketWrite => "decision_to_socket_write",
...
pub struct LatencyGateSnapshot {
    ...
    pub required_healthy_windows: u8,
}
...
impl LatencyAggregator {
    ...
    fn drain(&mut self, monitor: &LatencyMonitor, now_ns: u64) -> LatencySnapshot {
        ...
        let short_cutoff = now_ns.saturating_sub(10_000_000_000);
        let mut distributions = BTreeMap::new();
        let mut short_distributions = BTreeMap::new();
        ...
        if candidate.enforced && candidate.trading_allowed {
            self.healthy_windows = self.healthy_windows.saturating_add(1).min(3);
            if self.healthy_windows < 3 {
                candidate = blocked_gate(&self.config, "recovering", None, None);
            }
        } else if candidate.enforced {
            self.healthy_windows = 0;
        }
        candidate.healthy_windows = self.healthy_windows;
        snapshot.gate = candidate;
        snapshot
    }
}
...
fn initial_gate(config: &LatencyConfig, gate_enforced: bool) -> LatencyGateSnapshot {
    ...
    LatencyGateSnapshot {
        ...
        max_acceptable_p95_ms: config.max_acceptable_p95_ms,
...
fn evaluate_gate(
    ...
    for (mandatory_kind, minimum_samples) in [
        ...
        if mandatory.window_samples < minimum_samples {
            return blocked_gate(config, "warming_up", Some(mandatory_kind), mandatory.p95_ns);
        }
        if mandatory
            .sample_age_ms
    ...
    for kind in gated_kinds {
        ...
        {
            return blocked_gate(config, "p95_exceeded", Some(kind), distribution.p95_ns);
        }
    }
    LatencyGateSnapshot {
        enabled: true,
...
fn blocked_gate(
    ...
    LatencyGateSnapshot {
        ...
        observed_p95_ms: observed_p95_ns.map(|value| value as f64 / 1_000_000.0),
        healthy_windows: 0,
        required_healthy_windows: 3,
    }
}

fn distribution_since(
    ...
    let filtered = samples
        .iter()
        .copied()
        .filter(|sample| sample.observed_ns >= cutoff_ns)
        .collect::<VecDeque<_>>();
    distribution(&filtered, total_samples, now_ns)
}
...
mod tests {
    ...
    #[test]
    fn ten_second_and_long_windows_are_reported_independently() {
        let config = test_config(8, 60);
        let monitor = LatencyMonitor::new("SYN", 1, &config, false);
        monitor.record(LatencyKind::HotDecision, 10, 20_000_000_000);
        monitor.record(LatencyKind::HotDecision, 20, 35_000_000_000);
        let mut aggregator = LatencyAggregator::new("SYN", 1, config, false);
        let snapshot = aggregator.drain(&monitor, 40_000_000_000);
        assert_eq!(snapshot.distributions["hot_decision"].window_samples, 2);
        assert_eq!(
            snapshot.short_distributions["hot_decision"].window_samples,
            1
        );
        assert_eq!(
            snapshot.short_distributions["hot_decision"].p50_ns,
            Some(20)
        );
    }
    ...
    fn gate_warms_up_opens_and_blocks_on_bad_ack_latency() {
        ...
        let warming = aggregator.drain(&monitor, 4_000);
        assert!(!warming.gate.trading_allowed);
        assert_eq!(warming.gate.reason, "recovering");
        assert_eq!(warming.gate.healthy_windows, 1);
        let warming = aggregator.drain(&monitor, 5_000);
        assert!(!warming.gate.trading_allowed);
        assert_eq!(warming.gate.healthy_windows, 2);
        let open = aggregator.drain(&monitor, 6_000);
        assert!(open.gate.trading_allowed);
        assert_eq!(open.gate.reason, "ok");

        monitor.record(LatencyKind::SubmitToAck, 2_000_000, 7_000);
        let blocked = aggregator.drain(&monitor, 8_000);
        assert!(!blocked.gate.trading_allowed);
        assert_eq!(blocked.gate.reason, "p95_exceeded");
    ...
    fn production_gate_blocks_on_slow_websocket_rtt() {
        ...
        assert!(!snapshot.gate.trading_allowed);
        assert_eq!(snapshot.gate.reason, "p95_exceeded");
        assert_eq!(
            snapshot.gate.blocking_kind.as_deref(),
            Some("public_ws_ping_rtt")
        );
        assert_eq!(snapshot.gate.observed_p95_ms, Some(500.0));
    }
}
