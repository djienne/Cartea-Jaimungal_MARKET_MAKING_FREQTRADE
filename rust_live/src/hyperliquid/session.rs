...
impl HyperliquidSessionHandle {
    ...
    pub async fn enqueue_cancel_cloids(&self, cloids: Vec<String>) -> Result<()> {
        self.enqueue(SessionAction::CancelCloids(cloids)).await
    }
    ...
    pub async fn enqueue_schedule_cancel(&self, time: Option<u64>) -> Result<()> {
        self.enqueue(SessionAction::ScheduleCancel(time)).await
    }
    ...
    async fn enqueue(&self, action: SessionAction) -> Result<()> {
        self.commands
            .send(SessionCommand {
                action,
                response: None,
...
pub fn spawn_session(
    ...
    let task = tokio::spawn(run_session(args, command_rx, healthy, drop_next_response));
    (handle, task)
}
    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    let mut ping = tokio::time::interval(Duration::from_millis(5_000));
...
fn subscriptions(args: &SessionSpawnArgs) -> Vec<serde_json::Value> {
    vec![
        json!({"type": "orderUpdates", "user": args.credentials.account()}),
        json!({"type": "userFills", "user": args.credentials.account(), "aggregateByTime": false}),
        json!({"type": "userFundings", "user": args.credentials.account()}),
        json!({"type": "clearinghouseState", "user": args.credentials.account(), "dex": args.instrument.dex}),
        json!({"type": "openOrders", "user": args.credentials.account(), "dex": args.instrument.dex}),
        json!({"type": "activeAssetData", "user": args.credentials.account(), "coin": args.instrument.symbol}),
        json!({"type": "userNonFundingLedgerUpdates", "user": args.credentials.account()}),
        json!({"type": "notification", "user": args.credentials.account()}),
    ]
}
