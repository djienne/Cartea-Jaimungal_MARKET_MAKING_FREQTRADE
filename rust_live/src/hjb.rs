...
pub struct HjbSurface {
    ...
    pub t_grid: Vec<f64>,
    pub h_surface: Vec<Vec<f64>>,
    pub delta_plus_surface: Vec<Vec<f64>>,
    pub delta_minus_surface: Vec<Vec<f64>>,
}
...
impl HjbSurface {
    ...
    pub fn depth(&self, side: Side, q_exact: f64, tau_remaining: f64) -> Option<f64> {
        ...
        let surface = match side {
            Side::Buy => &self.delta_minus_surface,
            Side::Sell => &self.delta_plus_surface,
        };
        let q_grid: Vec<f64> = (self.q_min..=self.q_max).map(|q| q as f64).collect();
        let (q_lo, q_hi, q_weight) = bracket(&q_grid, q_exact);
        let low_row = interpolate_time(surface, t_lo, t_hi, t_weight, q_lo);
        let high_row = interpolate_time(surface, t_lo, t_hi, t_weight, q_hi);
        blend_depth(low_row, high_row, q_weight)
    }

    pub fn t0_depths(&self) -> (&[f64], &[f64]) {
        (&self.delta_plus_surface[0], &self.delta_minus_surface[0])
    }
}
...
pub fn solve_asymmetric(
    ...
    let mut delta_plus_surface = Vec::with_capacity(reverse_slices.len());
    let mut delta_minus_surface = Vec::with_capacity(reverse_slices.len());
    for slice in &reverse_slices {
        let (plus, minus) = depths_from_h(slice, parameters);
        delta_plus_surface.push(plus);
        delta_minus_surface.push(minus);
    }
...
fn interpolate_time(
    ...
    if low == high || weight <= 0.0 {
        ...
        surface[high][q_index]
    } else {
        let a = surface[low][q_index];
        let b = surface[high][q_index];
