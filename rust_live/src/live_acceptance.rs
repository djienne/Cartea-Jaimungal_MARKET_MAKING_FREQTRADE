...
use mm_live::config::{AppConfig, LiveMode};
use mm_live::execution::{AccountStateProvider, ExecutionBackend, HyperliquidLiveBackend};
use mm_live::hyperliquid::exchange::ActionOutcome;
use mm_live::hyperliquid::live_state::LiveOrderStatus;
use mm_live::hyperliquid::session::SessionEvent;
use mm_live::instrument::InstrumentSpec;
use mm_live::latency::{LatencyMonitor, LatencyObserver};
use mm_live::types::{unix_ms, ProcessClock, Side};
use std::sync::Arc;
            ...
            tokio::select! {
                ...
                event = events.recv() => {
                    ...
                    let maker_fill_seen = execution_events.iter().any(|event| {
                        matches!(event, mm_live::types::ExecutionEvent::Fill(fill) if fill.maker)
                    });
                    let taker_fill_seen = execution_events.iter().any(|event| {
                        matches!(event, mm_live::types::ExecutionEvent::Fill(fill) if !fill.maker)
                    });
                    if taker_fill_seen {
                        backend.enqueue_cancel_all_bot_orders().await?;
                        backend.market_close().await?;
