...
class ReplayVariant:
    ...
    # Adverse selection stress, as a MULTIPLE of the epsilon actually measured.
    #
    # This was an absolute price offset (epsilon_add), which is only meaningful
    # at the scale it was chosen for. It was chosen for ETH: mid ~4300, kappa
    # ~2-8, so +0.01 of price is a couple of bps and kappa*epsilon stays near the
    # 1.5 toxicity ceiling. On CASHCAT -- mid 0.1179, kappa 6200-8500 -- the same
    # 0.01 is 848 bps of mid and a toxicity of 50-68, and 0.02 is 149-204. Both
    # values drive every quote onto the 80 bps cap (and the opposite side onto
    # the 1.5 bps floor), so params_soft and params_hard returned BYTE-IDENTICAL
    # metrics: pnl +38.15 from 16 fills, all on the ask, on a pinned CASHCAT tape
    # on 2026-08-17. Two stress variants that always agree test nothing, and the
    # report counted them as two passes.
    #
    # A multiple is the direct statement of the thing being stressed -- "what if
    # adverse selection is worse than we measured" -- and it is scale-free in the
    # only way that matters here, because it inherits the instrument's own units.
    # A bps-of-mid stress would also be scale-free but ignores kappa, and it is
    # kappa*epsilon that the model and the validation gate respond to.
    #
    # 2x and 4x are empirically motivated, not round numbers: epsilon on the live
    # CASHCAT window moves by roughly a factor of 2-5 across measurement horizons
    # (eps+ 2.62 bps at 200 ms -> 6.12 bps at 5 s; eps- 1.10 -> 3.87 at 5 s ->
    # 5.02 at 30 s), so 2x and 4x span the real uncertainty in the input.
    epsilon_multiplier: float = 1.0
    # The old absolute offset, retained at 0.0 so any recorded artifact built on
    # epsilon_add=0.01/0.02 can still be regenerated by passing it explicitly.
    # Applied AFTER the multiplier. Not used by the shipped variants.
...
# Toxicity (kappa*epsilon) each parameter variant lands at with the 2026-08-17
# CASHCAT snapshot (kappa 6200/8500, epsilon 2.97e-5/1.19e-5), against the 1.5
# ceiling in validate_param_snapshot: baseline 0.184/0.101, params_soft
# 0.295/0.162, params_hard 0.884/0.486. No quote clamps to the cap in any of
# them; see tests/test_replay_report.py.
DEFAULT_VARIANTS = (
    ReplayVariant("baseline"),
    ReplayVariant("fee_2x", maker_fee_multiplier=2.0),
    ReplayVariant("latency_2x", latency_multiplier=2.0),
    ReplayVariant("params_soft", params_multiplier=0.8, epsilon_multiplier=2.0),
    ReplayVariant("params_hard", params_multiplier=1.2, epsilon_multiplier=4.0),
...
def variant_params(base: dict[str, float], variant: ReplayVariant) -> dict[str, float]:
    """Apply one variant's parameter stress to the measured snapshot.

    Epsilon is scaled first and offset second, so a variant that sets only the
    multiplier stays proportional to what the estimator measured on THIS
    instrument -- see the comment on ReplayVariant for why the absolute offset
    could not stay the default.
    """
    params = dict(base)
    ...
    for key in ("epsilon+", "epsilon-"):
        params[key] = float(params.get(key, 0.0)) * float(variant.epsilon_multiplier) + float(
            variant.epsilon_add
        )
    return params
...
def variant_config(base: ReplayConfig, variant: ReplayVariant) -> ReplayConfig:
    """Apply one stress to the base config, changing ONLY what the stress names.

    Built with dataclasses.replace rather than by re-listing the fields: the
    field-by-field copy this used to be silently reset every knob it had not
    heard of, so the whole HJB block (phi/alpha, horizon, time mode) and the
    inventory bounds ran at library defaults in every variant no matter what the
    operator configured. A stress test that quietly re-configures the model is
    not a stress test.
    """
...
def evaluate_metrics(
    ...
    *,
    min_days: float,
    q_max: int,
    # The long-only default this gate shipped with. A two-sided run (q_min < 0)
    # spends half its time at negative q, which is correct behaviour and would
    # otherwise be reported as inventory_out_of_bounds on every short state.
    q_min: int = 0,
...
def build_report(
    *,
    ...
    for variant in variants:
        variant_metrics_config = variant_config(config, variant)
        # Every variant scores the SAME bytes when a tape is supplied. Without
        # one each variant reloads, and the collector writes a new shard every
        # ~10 s, so "fee_2x is worse than baseline" could be nothing but the two
        # runs having seen different data.
        metrics: ReplayMetrics = (
            run_replay(variant_metrics_config, variant_params(params, variant))
            if tape is None
            else run_replay(variant_metrics_config, variant_params(params, variant), tape=tape)
        )
    ...
    return {
        ...
        "config": {
            ...
            "tape_pinned": tape is not None,
            "event_clock": config.event_clock,
            ...
            "total_quote_latency_ms": int(config.decision_latency_ms) + int(config.order_ack_latency_ms),
            "quote_refresh_interval_ms": config.quote_refresh_interval_ms,
            "q_min": config.q_min,
            "q_max": config.q_max,
            "hjb_q_min": config.hjb_q_min,
            "inventory_unit_base": config.inventory_unit_base,
            "hjb_phi_kappa_t": config.hjb_phi_kappa_t,
            "hjb_alpha_kappa": config.hjb_alpha_kappa,
            "hjb_phi_kappa_t_max": config.hjb_phi_kappa_t_max,
            "hjb_phi": config.hjb_phi,
            "hjb_alpha": config.hjb_alpha,
            "hjb_horizon_seconds": config.hjb_horizon_seconds,
            "hjb_time_mode": config.hjb_time_mode,
...
def parse_args() -> argparse.Namespace:
    ...
    # Latency and inventory knobs. These reach ReplayConfig fields that existed
    # for a long time with no way to set them, so every report ever generated ran
    # a long-only agent at 500 ms with library-default HJB penalties.
    parser.add_argument("--decision-latency-ms", type=int, default=None)
    parser.add_argument("--order-ack-latency-ms", type=int, default=None)
    parser.add_argument("--cancel-latency-ms", type=int, default=250)
    parser.add_argument(
        "--latency-ms",
        type=int,
        default=None,
    ...
    parser.add_argument("--hjb-q-min", type=int, default=None, help="Inventory domain the HJB is solved on.")
    parser.add_argument("--inventory-unit-base", type=float, default=0.01)
    parser.add_argument("--hjb-time-mode", choices=("episodic", "stationary"), default="episodic")
    parser.add_argument("--hjb-horizon-seconds", type=float, default=60.0)
    parser.add_argument("--hjb-alpha", type=float, default=0.001, help="Raw; used only when --hjb-alpha-kappa is 0.")
    parser.add_argument("--hjb-phi", type=float, default=0.0001, help="Raw; used only when --hjb-phi-kappa-t is 0.")
    parser.add_argument("--hjb-phi-kappa-t", type=float, default=0.0, help="Dimensionless phi*kappa*T; live runs 10.0.")
    parser.add_argument("--hjb-alpha-kappa", type=float, default=0.0, help="Dimensionless alpha*kappa; live runs 0.05.")
    parser.add_argument("--hjb-phi-kappa-t-max", type=float, default=QuoteConfig.hjb_phi_kappa_t_max)
    parser.add_argument("--sigma2-per-sec", type=float, default=None)
    parser.add_argument("--event-clock", choices=("exchange", "local"), default="exchange")
...
def main() -> int:
    ...
    # One load for all six variants: the variants differ by fee, latency, params
    # and tick, and nothing else may differ -- least of all which shards the
    # collector happened to have flushed when each one started.
    report = build_report(
        config=config,
        params=params,
