% Reference chapter extract used for equation checking. This is source material,
% not documentation of the repository's current runtime or configuration.
...
% Geometry settings to match standard textbook margins
\geometry{left=1in, right=1in, top=1in, bottom=1in}

% Custom commands for notation found in the text
\newcommand{\E}{\mathbb{E}}
\newcommand{\Ind}{\mathbf{1}}

\begin{document}
	
	\setcounter{chapter}{9} % So the next chapter is 10
	\chapter{Market Making}
	
	\section{Introduction}
	
	In this chapter we model how a \textbf{market maker} (MM) maximises terminal wealth by trading in and out of positions using limit orders (LOs). The MM provides liquidity to the limit order book (LOB) by posting buy and sell LOs and the control variable is the depth, which is measured from the midprice, at which these LOs are posted. To formalise the problem, we list the relevant variables that we use throughout this section:
	
	\begin{itemize}
		\item $S = (S_t)_{0 \leq t \leq T}$, denotes the midprice, with $S_t = S_0 + \sigma W_t$, $\sigma > 0$ and $W = (W_t)_{0 \leq t \leq T}$ is a standard Brownian motion,
		\item $\delta^\pm = (\delta^\pm_t)_{0 \leq t \leq T}$ denote the depth at which the agent posts LOs; sell LOs are posted at a price of $S_t + \delta^+_t$ and buy LOs at a price of $S_t - \delta^-_t$,
		\item $M^\pm = (M^\pm_t)_{0 \leq t \leq T}$ denote the counting processes corresponding to the arrival of other participants' buy ($+$) and sell ($-$) market orders (MOs) which arrive at Poisson times with intensities $\lambda^\pm$,
		\item $N^{\delta, \pm} = \left(N^{\delta, \pm}_t\right)_{0 \leq t \leq T}$ denote the controlled counting processes for the agent's filled sell ($+$) and buy ($-$) LOs,
		\item conditional on a market order (MO) arrival, the posted LO is filled with probability $e^{-\kappa^\pm \delta^\pm_t}$, with $\kappa^\pm \geq 0$,
		\item $X^\delta = (X^\delta_t)_{0 \leq t \leq T}$ denotes the MM's cash process and satisfies the SDE
		\begin{equation}
			dX^\delta_t = (S_t - \delta^-_t) \, dN^{\delta, -}_t + (S_t + \delta^+_t) \, dN^{\delta, +}_t, \label{eq:10.1}
		\end{equation}
		...
		\item $Q^\delta = (Q^\delta_t)_{0 \leq t \leq T}$ denotes the agent's inventory process and
		\begin{equation}
			Q^\delta_t = N^{\delta, -}_t - N^{\delta, +}_t. \label{eq:10.2}
		\end{equation}
	\end{itemize}
	
	As discussed in Section 8.2, whenever the process $N^{\delta, \pm}$ jumps, the process $M^{\delta, \pm}$ must also jump; but when $M^{\delta, \pm}$ jumps, $N^{\delta, \pm}$ will jump only if the MO is large enough to fill the agent's LO, and $N^{\delta, \pm}$ is not a Poisson process. Moreover, note that the fill rate of LOs can be written as $\Lambda^{\delta, \pm}_t = \lambda^\pm e^{-\kappa_\pm \delta^\pm_t}$, which is the rate of execution of an LO.
	...
	\begin{equation}
		H(t, x, S, q) = \sup_{\delta^\pm \in \mathcal{A}} H^\delta(t, x, S, q), \label{eq:10.3}
	\end{equation}
	...
	\begin{equation}
		\begin{aligned}
			0 &= \partial_t H + \tfrac{1}{2}\sigma^2 \partial_{SS} H - \phi q^2 \\
			&\quad + \lambda^+ \sup_{\delta^+} \left\{ e^{-\kappa^+ \delta^+} \left( H(t, x + (S + \delta^+), q - 1, S) - H \right) \right\} \Ind_{q > \underline{q}} \\
			&\quad + \lambda^- \sup_{\delta^-} \left\{ e^{-\kappa^- \delta^-} \left( H(t, x - (S - \delta^-), q + 1, S) - H \right) \right\} \Ind_{q < \bar{q}},
		\end{aligned} \label{eq:10.4}
	\end{equation}
	where $\Ind$ is the indicator function, with terminal condition
	\begin{equation}
		H(T, x, S, q) = x + q(S - \alpha q). \label{eq:10.5}
	\end{equation}
	
	Recall that the set of admissible strategies imposes bounds on $q_t$, so that when $q_t = \bar{q}$ ($\underline{q}$) the optimal strategy is to post one-sided LOs which are obtained by solving (\ref{eq:10.4}) with the term proportional to $\lambda^-$ ($\lambda^+$) absent as enforced by the indicator function $\Ind$ in the DPE. Alternatively, one can view these boundary cases as imposing $\delta^- = +\infty$ and $\delta^+ = +\infty$ when $q = \bar{q}$ and $\underline{q}$ respectively.
	
	Intuitively, the various terms in the DPE represent the arrival of MOs that may be filled by LOs together with the diffusion of the asset price through the term $\tfrac{1}{2}\sigma^2 \partial_{SS} H$, and the effect of penalising deviations of inventories from zero along the entire path of the strategy which is captured by the term $\phi q^2$. In the second line of the DPE the sup over $\delta^+$ contain the terms due to the arrival of a market buy order (which is filled by a limit sell order), and here we see the change in the value function $H$ due to the arrival of the MO which fills the LO, so that cash increases by $(S+\delta^+)$ and inventory decreases by one unit. Similarly, in the last line in the DPE the sup over $\delta^-$ contain the analogous terms for the market sell orders which are filled by limit buy orders.
	
	To solve the DPE we use the terminal condition (\ref{eq:10.5}) to make an ansatz for $H$. In particular, we write
	\begin{equation}
		H(t, x, q, S) = x + q S + h(t, q), \label{eq:10.6}
	\end{equation}
	...
	We proceed by substituting the ansatz into (\ref{eq:10.4}) to obtain
	\begin{equation}
		\begin{aligned}
			\phi q^2 &= \partial_t h(t, q) + \lambda^+ \sup_{\delta^+} \left\{ e^{-\kappa^+ \delta^+} \left( \delta^+ + h(t, q-1) - h(t, q) \right) \right\} \Ind_{q > \underline{q}} \\
			&\quad + \lambda^- \sup_{\delta^-} \left\{ e^{-\kappa^- \delta^-} \left( \delta^- + h(t, q+1) - h(t, q) \right) \right\} \Ind_{q < \bar{q}},
		\end{aligned} \label{eq:10.7}
	\end{equation}
	with terminal condition $h(T, q) = -\alpha q^2$.
	Then the optimal depths in feedback form are given by
	\begin{subequations}
		\begin{align}
			\delta^{+, *}(t, q) &= \frac{1}{\kappa^+} - h(t, q-1) + h(t, q), \quad q \neq \underline{q}, \label{eq:10.8a} \\
			\delta^{-, *}(t, q) &= \frac{1}{\kappa^-} - h(t, q+1) + h(t, q), \quad q \neq \bar{q}, \label{eq:10.8b}
		\end{align}
	\end{subequations}
	and the boundary cases are $\delta^{+, *}(t, q) = +\infty$ and $\delta^{-, *}(t, q) = +\infty$ when $q = \bar{q}$ and $\underline{q}$ respectively.
	
	To understand the intuition behind the feedback controls we first note that the optimal $\delta^\pm$ can be decomposed into two components. The first component, $1/\kappa^\pm$, is the optimal strategy for a MM who does not impose any restrictions on inventory ($\alpha = \phi = 0$ and $|\underline{q}| = \bar{q} = \infty$) – see below in subsection 10.2.1.
	...
	\begin{equation}
		\phi q^2 = \partial_t h(t, q) + \frac{e^{-1}\lambda^+}{\kappa^+} e^{-\kappa^+( -h(t,q-1)+h(t,q) )} \Ind_{q>\underline{q}} + \frac{e^{-1}\lambda^-}{\kappa^-} e^{-\kappa^-( -h(t,q+1)+h(t,q) )} \Ind_{q<\bar{q}}. \label{eq:10.9}
	\end{equation}
	
	\textbf{Solving the DPE}
	...
	\[
	\bm{\omega}(t) = [\omega(t, \bar{q}), \omega(t, \bar{q}-1), \dots, \omega(t, \underline{q})]'.
	\]
	Now, let $\mathbf{A}$ denote the $(\bar{q} - \underline{q} + 1)$-square matrix whose rows are labelled from $\bar{q}$ to $\underline{q}$ and whose entries are given by
	\begin{equation}
		\mathbf{A}_{i,q} = \begin{cases}
			\phi \kappa q^2, & i = q, \\
			\lambda^+ e^{-1}, & i = q - 1, \\
			\lambda^- e^{-1}, & i = q + 1, \\
			0, & \text{otherwise},
		\end{cases} \label{eq:10.10}
	\end{equation}
	with terminal and boundary conditions $\omega(T, q) = e^{-\alpha \kappa q^2}$.
	Then (\ref{eq:10.9}) becomes
	\[
	\partial_t \bm{\omega}(t) + \mathbf{A} \, \bm{\omega}(t) = \mathbf{0}.
	\]
	The solution of this matrix ODE is straightforward and we finally have,
	\begin{equation}
		\boxed{\bm{\omega}(t) = e^{\mathbf{A}(T-t)} \mathbf{z},} \label{eq:10.11}
	\end{equation}
	where $\mathbf{z}$ is a $(\bar{q} - \underline{q} + 1)$-dim vector where each component is $z_j = e^{-\alpha \kappa j^2}$, $j = \bar{q}, \dots, \underline{q}$.
	
	\textbf{Behaviour of the Strategy}
	
	Figure 10.1 shows the behaviour of the optimal depths as a function of time for different inventory levels. In the examples the arrival rate of MOs is $\lambda^\pm = 1$ (there are on average 1 buy and 1 sell MO per second), $\bar{q} = -\underline{q} = 3$, and $\phi = 10^{-5}$ in panel (a), and $\phi = 2 \times 10^{-4}$ in panel (b). In the left of panel (a) we show the optimal sell postings $\delta^+$, i.e. upon the arrival of a market buy order the MM is willing to sell one unit of the asset at the price $S_t + \delta^+$, and in the right of panel (a) we show the optimal buy postings $\delta^-$. For example, when the strategy is far away from expiry and inventories are close to the allowed minimum, the optimal sell posting is furthest away from the midprice because only at a very `high' price is the MM willing to decrease her inventories further, and at the same time the optimal buy posting is very close to the midprice because the strategy would like to complete round-trip trades (i.e. a buy followed by a sell or a sell followed by a buy) and push inventories to zero.
	...
	It is also interesting to see that the optimal strategy induces mean reversion in inventories. For example, if $q_t = 2$ then the sell depth is lower than the buy depth, $\delta^+ < \delta^-$, so that it is more likely for the strategy to sell, than to buy, one unit of the asset. This asymmetry in the optimal depths is what induces mean reversion to zero in the inventory. Moreover, in panel (b) the strategy's running inventory penalty is much higher and it is clear that the higher $\phi$ is, the quicker inventories will revert to zero.
	...
	\begin{equation}
		\begin{aligned}
			\mu(t, q) &\triangleq \lim_{s \downarrow t} \frac{1}{s-t} \E \left[ Q_s - Q_t \,|\, Q_t- = q \right] \\
			&= \lambda^- e^{-\kappa^- \delta^{-, *}(t,q)} - \lambda^+ e^{-\kappa^+ \delta^{+, *}(t,q)}.
		\end{aligned} \label{eq:10.12}
	\end{equation}
	...
	Now we turn to discussing the financial performance of the strategy. The left panel of Figure 10.4 shows the profit and loss (P\&L) of the optimal strategy and the right panel shows the lifetime inventory for different running penalty parameters $\phi = \{10^{-5}, 5 \times 10^{-5}, 10^{-3}, 10^{-2}\}$. We observe that when $\phi$ increases the histogram of P\&L shifts to the left because the strategy does not allow inventory positions to stray away from zero, and hence expected profits decrease. The lifetime inventory histogram shows how much time the strategy holds an inventory of $n$. For example, when $\phi = 10^{-2}$ we know that the strategy heavily penalises deviations of running inventory from zero, so the strategy spends most of the time at inventory levels of $-1, 0, 1$. As the running inventory penalty becomes smaller, the strategy spends more time at levels away from zero.
	
	\subsection{Market Making with no Inventory Restrictions} \label{sec:10.2.1}
	
	If we assume that the MM does not penalise running inventories, does not pick up a terminal inventory penalty, that is $\phi = \alpha = 0$, and there are no constraints on the amount of inventory the strategy may accumulate, i.e $|\underline{q}|, \bar{q} \to \infty$, then the MM's strategy simplifies to
	\begin{equation}
		\boxed{\delta^{+, *}(t, q) = \frac{1}{\kappa^+}, \quad \text{and} \quad \delta^{-, *}(t, q) = \frac{1}{\kappa^-}.} \label{eq:10.13}
	\end{equation}
	...
	\begin{equation}
		H(t, x, q, S) = x + q S + h(t). \label{eq:10.14}
	\end{equation}
	This is similar to the one proposed above, see (\ref{eq:10.6}), but here $h(t)$ does not depend on $q$ because the MM does not pose any restrictions on inventory throughout the life of the strategy and can liquidate terminal inventory at the midprice. Thus, substituting the ansatz into the DPE
	\begin{equation}
		0 = \partial_t h + \lambda^+ \sup_{\delta^+} \left\{ e^{-\kappa^+ \delta^+} \delta^+ \right\} + \lambda^- \sup_{\delta^-} \left\{ e^{-\kappa^- \delta^-} \delta^- \right\} \label{eq:10.15}
	\end{equation}
	with terminal condition $h(T) = 0$, delivers the result (\ref{eq:10.13}).
	...
	\begin{equation}
		e^{-\kappa^\pm \delta^\pm} - \kappa^\pm \delta^\pm e^{-\kappa^\pm \delta^\pm} = 0, \label{eq:10.16}
	\end{equation}
	so the optimal depths are as in (\ref{eq:10.13}).
	
	\subsection{Market Making At-The-Touch} \label{sec:10.2.2}
	
	In very liquid markets, most orders do not walk the book and instead tend to only lift or hit LOs posted at-the-touch. To capture this market feature, in this section we investigate the agent's optimal postings at-the-touch, i.e. at the best bid and best offer. Throughout we assume that the spread is constant and equal to $\Delta$. Next, let $\ell^\pm_t \in \{0, 1\}$ denote whether the agent is posted on the sell side (+) or buy side (-) of the LOB. In this way, the agent may be posted on both sides of the book, only the sell side, only the buy side, or not posted at all. Her performance criteria is
	\[
	H^\ell(t, x, S, q) = \E_{t,x,S,q} \left[ X^\ell_T + Q^\ell_T \left(S_T - (\tfrac{\Delta}{2} + \varphi Q^\ell_T)\right) - \phi \int_t^T (Q^\ell_u)^2 \, du \right],
	\]
	where her cash process $X^\ell_t$ now satisfies the SDE
	\[
	dX^\ell_t = (S_t + \tfrac{\Delta}{2}) \, dN^{+, \ell}_t - (S_t - \tfrac{\Delta}{2}) \, dN^{-, \ell}_t,
	\]
	where $N^{\pm, \ell}_t$ denote the counting process for filled LOs. We also further assume that, if she is posted in the LOB, when a matching MO arrives her LO is filled with probability one. In this case, $N^{\pm, \ell}_t$ are controlled doubly stochastic Poisson processes with intensity $\ell^\pm_t \lambda^\pm$. Finally, at the terminal date any open inventory position is liquidated using an MO and the price obtained per share is the best bid ($Q_T > 0$) or offer ($Q_T < 0$) and picks up a penalty $\varphi Q_T^2$, with $\varphi \geq 0$, which includes market impact (walking the LOB) and liquidity taking fees.
	
	As before, the set $\mathcal{A}$ of admissible strategies are $\mathcal{F}$-predictable such that the agent is not posted on the buy (sell) side if her inventory is equal to the upper (lower) inventory constraints $\bar{q}$ ($\underline{q}$) and her value function is denoted by
	\[
	H(t, x, S, q) = \sup_{\ell \in \mathcal{A}} H^\ell(t, x, S, q).
	\]
	
	\textbf{The Resulting DPE}
	...
	\begin{align*}
		...
		&\quad + \lambda^+ \max_{\ell^+ \in \{0,1\}} \left\{ \left( H\left(t, x + (S + \tfrac{\Delta}{2})\ell^+, S, q - \ell^+\right) - H \right) \right\} \Ind_{q > \underline{q}} \\
		&\quad + \lambda^- \max_{\ell^- \in \{0,1\}} \left\{ \left( H\left(t, x - (S - \tfrac{\Delta}{2})\ell^-, S, q + \ell^-\right) - H \right) \right\} \Ind_{q < \bar{q}},
	\end{align*}
	subject to the terminal condition
	\[
	H(T, x, S, q) = x + q \left(S - (\tfrac{\Delta}{2} + \varphi q)\right).
	\]
	The various terms in the DPE carry the following interpretations:
	\begin{itemize}
		\item the first line in the DPE represents the diffusive component of the midprice and the running inventory penalisation,
		\item the maximisation terms represent the agent's control to post or not on the sell or buy side of the LOB,
		\item the maximisation term in the second line represents the change in value function, if the agent is posted, due to the arrival of an MO which lifts the agent's offer,
		\item the third line is for the other side of the book.
	\end{itemize}
	...
	\begin{align*}
		...
		&\quad + \lambda^+ \max_{\ell^+ \in \{0,1\}} \left\{ \left( \ell^+ \tfrac{\Delta}{2} + \left[ h(t, q - \ell^+) - h(t, q) \right] \right) \right\} \Ind_{q > \underline{q}} \\
		&\quad + \lambda^- \max_{\ell^- \in \{0,1\}} \left\{ \left( \ell^- \tfrac{\Delta}{2} + \left[ h(t, q + \ell^-) - h(t, q) \right] \right) \right\} \Ind_{q < \bar{q}},
	\end{align*}
	subject to the terminal condition
	\[
	...
	\begin{equation}
		\boxed{
			\begin{aligned}
				\ell^{+, *}(t, q) &= \Ind_{\left\{ \frac{\Delta}{2} + [h(t, q-1) - h(t, q)] > 0 \right\} \cap \{q > \underline{q}\}}, \\
				\ell^{-, *}(t, q) &= \Ind_{\left\{ \frac{\Delta}{2} + [h(t, q+1) - h(t, q)] > 0 \right\} \cap \{q < \bar{q}\}}.
			\end{aligned}
		} \label{eq:10.17}
	\end{equation}
	...
	\[
	T = 300 \text{ sec}, \quad \bar{q} = -\underline{q} = 20, \quad \lambda^\pm = \frac{50}{300},
	\]
	\[
	...
	\[
	H^\ell(t, x, S, q) = \E_{t,x,S,q} \left[ X^\ell_T + Q^\ell_T (S_T - (\tfrac{\Delta}{2} + \varphi Q^\ell_T)) - \phi \int_t^T (Q^\ell_u)^2 \, du \right],
	\]
	and her cash process $X^\ell_t$ now satisfies the SDE
	\[
	dX^\ell_t = (S_t + \tfrac{\Delta}{2}) \, \ell^+_t \, dN^{+, \ell}_t - (S_t - \tfrac{\Delta}{2}) \, \ell^-_t \, dN^{-, \ell}_t,
	\]
	where $\ell^\pm_t$ are $\mathcal{F}$-predictable such that
	\[
	\ell^+_t \in \{0, 1, 2, \dots, q_t - \underline{q} \} \quad \text{and} \quad \ell^-_t \in \{0, 1, 2, \dots, \bar{q} - q_t \},
	\]
	and $N^{\pm, \ell}_t$ denote the counting processes for her filled LOs – not accounting for the volume traded (i.e. it only counts whether an MO arrived and filled her posted LO). The restrictions on the volume ensure that the agent never posts a volume which, if filled, would send her inventory outside of her allowed trading bounds. We further assume that if she is posted in the LOB when a matching MO arrives, her LO is filled with probability $\rho(\ell)$ where $\ell$ is posted volume. For example, $\rho(\ell) = e^{-\kappa \ell}$ would represent an exponential fill probability. In this case, $N^{\pm, \ell}_t$ are controlled doubly stochastic Poisson processes with intensity $\rho(\ell_t^\pm)\lambda^\pm \Ind_{\ell_t^\pm > 0}$. In this formulation, we have further assumed that if the agent makes a post of a given volume, the entire volume is matched or none at all is matched. The approach here can be generalised to account for partial fills of postings, but this is left as an exercise for the reader.
	
	As before, the set $\mathcal{A}$ of admissible strategies also restricts her so that the strategy does not post on the buy (sell) side of the LOB if her inventory is equal to the upper (lower) inventory constraints $\bar{q}$ ($\underline{q}$). Her value function is denoted by
	\[
	H(t, x, S, q) = \sup_{\ell \in \mathcal{A}} H^\ell(t, x, S, q).
	\]
	...
	\begin{align*}
		0 &= \left( \partial_t + \tfrac{1}{2}\sigma^2 \partial_{SS} \right) H - \phi q^2 \\
		&\quad + \lambda^+ \max_{\ell^+ \in \{0, 1, \dots, q-\underline{q}\}} \left\{ \rho(\ell^+) \left( H\left(t, x + (S + \tfrac{\Delta}{2})\ell^+, S, q - \ell^+\right) - H \right) \right\} \Ind_{q > \underline{q}} \\
		&\quad + \lambda^- \max_{\ell^- \in \{0, 1, \dots, \bar{q}-q\}} \left\{ \rho(\ell^-) \left( H\left(t, x - (S - \tfrac{\Delta}{2})\ell^-, S, q + \ell^-\right) - H \right) \right\} \Ind_{q < \bar{q}},
	\end{align*}
	subject to the terminal condition
	\[
	H(T, x, S, q) = x + q (S - (\tfrac{\Delta}{2} + \varphi q)).
	\]
	With the exception that the maxima are computed over the set of allowed volumes and cash jumps accordingly, this non-linear PDE is identical to the one derived in the previous section when the agent did not optimise over the volume of the LOs. The terminal condition suggests the usual ansatz $H(t, x, S, q) = x + q S + h(t, q)$ which splits out the accumulated cash, book value of the inventory marked-to-market using the midprice, and the added value from optimally making markets throughout the remaining life of the strategy. On substituting into the DPE we find that $h$ satisfies
	\begin{align*}
		0 &= \partial_t h - \phi q^2 \\
		&\quad + \lambda^+ \max_{\ell^+ \in \{0, 1, \dots, q-\underline{q}\}} \left\{ \rho(\ell^+) \left( \ell^+ \tfrac{\Delta}{2} + [h(t, q - \ell^+) - h(t, q)] \right) \right\} \Ind_{q > \underline{q}} \\
		&\quad + \lambda^- \max_{\ell^- \in \{0, 1, \dots, \bar{q}-q\}} \left\{ \rho(\ell^-) \left( \ell^- \tfrac{\Delta}{2} + [h(t, q + \ell^-) - h(t, q)] \right) \right\} \Ind_{q < \bar{q}},
	\end{align*}
	subject to the terminal condition
	\[
	h(T, q) = -q \left(\tfrac{\Delta}{2} + \varphi q\right).
	...
	\[
	T = 10 \text{ sec}, \quad \bar{q} = -\underline{q} = 20, \quad \lambda^+ = 5,
	\]
	\[
	...
	\]
	Notice that the posted sell volume increases as inventory increases, while the posted buy volume increases as inventory decreases. Furthermore, there are large regions where the agent is posted only on one side of the LOB. Figure 10.7 shows the asymptotic optimal volume postings at $t \to 0$ for the agent who posts only at-the-touch. The left panel shows both the buy and sell postings when $\phi = 10^{-3}$, while the right panel shows how the postings vary when $\phi$ varies for the sell side of the book. For $\phi = 10^{-3}$, the agent stops posting limit sell orders when her inventory is at or below $-7$ and stops posting limit buy orders when her inventory is at or above $+7$. As the right panel in Figure 10.7 shows, the critical level below which the agent stops posting moves towards zero inventory as the inventory penalisation parameter $\phi$ increases. With $\phi = 0.1$, the agent will withdraw from the market and simply not post any limit orders.
	...
	\section{Utility Maximising Market Maker}
	...
	\[
	G^\delta (t, x, S, q) = \E_{t,x,S,q} \left[ -\exp \left\{ -\gamma \left( X^\delta_T + Q^\delta_T (S_T - \alpha Q^\delta_T) \right) \right\} \right].
	\]
	...
	\[
	\begin{cases}
		\left( \partial_t + \tfrac{1}{2}\sigma^2 \partial_{SS} \right) G \\
		\quad + \lambda^+ \sup_{\delta^+} \left\{ e^{-\kappa^+ \delta^+} \left[ G(t, x + (S + \delta^+), S, q - 1) - G \right] \Ind_{q > \underline{q}} \right\} \\
		\quad + \lambda^- \sup_{\delta^-} \left\{ e^{-\kappa^- \delta^-} \left[ G(t, x - (S - \delta^-), S, q + 1) - G \right] \Ind_{q < \bar{q}} \right\} \\
		\qquad = 0, \\
		G(t, x, S, 0) \qquad = -e^{-\gamma x}, \\
		G(T, x, S, q) \qquad = -e^{-\gamma (x + q(S - \alpha q))}.
	\end{cases}
	\]
	We leave it as an exercise for the reader to show that ansatz
	\[
	G(t, x, S, q) = -e^{-\gamma (x + q S + g(t, q))}
	\]
	leads to the following equation for $g(t, q)$:
	\begin{equation}
		\begin{aligned}
			\partial_t g - \tfrac{1}{2}\sigma^2 \gamma q^2 &+ \sup_{\delta^+} \lambda^+ e^{-\kappa^+ \delta^+} \frac{1 - e^{-\gamma (\delta^+ + g(t, q-1) - g(t, q))}}{\gamma} \\
			&+ \sup_{\delta^-} \lambda^- e^{-\kappa^- \delta^-} \frac{1 - e^{-\gamma (\delta^- + g(t, q+1) - g(t, q))}}{\gamma} = 0,
		\end{aligned} \label{eq:10.18}
	\end{equation}
	with terminal and boundary conditions
	\[
	g(t, 0) = 0, \quad \text{and} \quad g(T, q) = -\alpha q^2.
	\]
	From the first order condition, we find that the optimal depths, in feedback control form, at which the agent posts are
	\begin{equation}
		\boxed{
			\begin{aligned}
				\delta^{*, +} &= \frac{1}{\gamma} \log \left( 1 + \frac{\gamma}{\kappa^+} \right) + g(t, q) - g(t, q-1), \quad & q > \underline{q}, \\
				\delta^{*, -} &= \frac{1}{\gamma} \log \left( 1 + \frac{\gamma}{\kappa^-} \right) + g(t, q) - g(t, q+1), \quad & q < \bar{q}.
			\end{aligned}
		} \label{eq:10.19}
	\end{equation}
	
	This form is very similar to, but slightly differs from, the optimal depth in the previous section provided in (\ref{eq:10.8a}). The $g$ function may differ from $h$ and the base line level $(\kappa^\pm)^{-1}$ is modified to $(\hat{\kappa}^\pm)^{-1} = \frac{1}{\gamma} \log(1 + \frac{\gamma}{\kappa^\pm})$. This modification can be seen as a risk aversion bias. Indeed, in the limit of zero risk-aversion
	\[
	\hat{\kappa}^\pm \xrightarrow{\gamma \downarrow 0} \kappa^\pm,
	\]
	and the result from the previous section is recovered.
	
	Substituting this feedback form into (\ref{eq:10.18}), we now find the non-linear ODE
	\[
	\partial_t g - \tfrac{1}{2}\sigma^2 \gamma q^2 + \frac{\hat{\lambda}^+}{\hat{\kappa}^+} e^{-\kappa^+ (g(t, q) - g(t, q-1))} + \frac{\hat{\lambda}^-}{\hat{\kappa}^-} e^{-\kappa^- (g(t, q) - g(t, q+1))} = 0,
	\]
	where
	\[
	\hat{\lambda}^\pm = \left( \kappa^\pm / (\kappa^\pm + \gamma) \right)^{1 + \kappa^\pm / \gamma} \lambda^\pm.
	\]
	In the limit of zero risk-aversion $\hat{\lambda}^\pm \xrightarrow{\gamma \downarrow 0} e^{-1}\lambda^\pm = \tilde{\lambda}^\pm$ and once again we recover the parameter that appears in (\ref{eq:10.9}). The above ODE is in fact identical in structure to (\ref{eq:10.9}). Hence, matching parameters by setting
	\[
	...
	\begin{equation}
		\boxed{\delta^{\pm, *} = \delta^{\pm, *}_0 + \left( (\hat{\kappa}^\pm)^{-1} - (\kappa_0^\pm)^{-1} \right).} \label{eq:10.20}
	\end{equation}
	...
	\[
	H(t, x, S, q) = -\frac{1}{\gamma} \log(-G(t, x, S, q)),
	\]
	or writing the value function in the original control form, we obtain
	\begin{equation}
		\boxed{
			\begin{aligned}
				&\sup_{\delta \in \mathcal{A}} \E_{t,x,S,q} \left[ X^\delta_T + Q^\delta_T (S_T - \alpha Q_T) - \phi \int_0^T (Q^\delta_s)^2 \, ds \right] \\
				&\quad = -\frac{1}{\gamma} \log \left( -\sup_{\delta \in \mathcal{A}} \E_{t,x,S,q} \left[ -\exp \left\{ -\gamma \left( X^\delta_T + Q^\delta_T (S_T - \alpha Q_\tau) \right) \right\} \right] \right).
			\end{aligned}
		} \label{eq:10.21}
	\end{equation}
	...
	\section{Market Making with Adverse Selection}
	
	The market place is populated by traders that come to the market for different reasons and with varying degrees of information. One of the most important risks faced by MMs is adverse selection risk. As discussed in Chapter 2 (see for example Sections 2.2 and 2.3), when trading with informed market participants the MM is exposed to having a sell limit order (LO) filled right before prices go up, or a buy LO filled before prices go down. In this section we extend the market making problem developed in Section 10.2, and present two ways in which midprice dynamics incorporate adverse selection effects. In the first model we assume that the midprice undergoes jumps in the direction of incoming market orders (MOs). In the second specification the midprice's drift has a short-term-alpha component which is affected by the arrival of MOs.
	
	\subsection{Impact of Market Orders on Midprice} \label{sec:10.4.1}
	
	Here we assume that the midprice dynamics follows
	\begin{equation}
		dS_t = \sigma dW_t + \epsilon^+ dM^+_t - \epsilon^- dM^-_t, \label{eq:10.22}
	\end{equation}
	where $M^+_t$ and $M^-_t$ are Poisson processes, with intensities $\lambda^+$ and $\lambda^-$ respectively, which count the number of buy ($+$) and sell ($-$) MOs. Every time an MO arrives, the midprice will undergo a jump of size $\epsilon^\pm$ which are i.i.d. and whose distribution functions are $F^\pm$ with finite first moment denoted by $\bar{\epsilon}^\pm = \E[\epsilon^\pm]$.
	
	Intuitively, here we can view the dynamics of the midprice as the sum of two components. The first component, the Brownian motion on the right-hand side of (\ref{eq:10.22}), captures the changes in the midprice that are due to information flows that reach all or some market participants who subsequently update their quotes. The other component, the jump process with increments $\epsilon^+ dM^+_t - \epsilon^- dM^-_t$, represents the changes in the midprice caused by the arrival of MOs that have a permanent price impact. MOs may come at times when there is enough liquidity in the market – hence prices remain unchanged or change by a negligible amount; or they may arrive at times when liquidity is thin or the orders are sent by traders with superior information, and these trades have a permanent impact on prices. The impact of trading on the midprice may also be viewed as the action of informed traders. If an informed trader purchases (sells) shares, he will only do so if the asset price is known to be going up (down). The resulting increase (decrease) of the midprice following informed trading can be approximated by an immediate, and permanent, price impact as we model here.
	
	The rest of the MM's setup is as in Section 10.2, with the only difference that here the midprice follows (\ref{eq:10.22}). For convenience we repeat the MM's performance criteria:
	\[
	H^\delta(t, x, S, q) = \E_{t,x,S,q} \left[ X^\delta_T + Q^\delta_T (S_T - \alpha Q^\delta_T) - \phi \int_t^T (Q^\delta_u)^2 \, du \right],
	\]
	...
	\begin{equation}
		\begin{aligned}
			0 &= \partial_t H + \tfrac{1}{2}\sigma^2 \partial_{SS} H - \phi q^2 \\
			&\quad + \lambda^+ \sup_{\delta^+} \left\{ e^{-\kappa^+ \delta^+} \Ind_{q > \underline{q}} \E \left[ H(t, x + (S + \delta^+), S + \epsilon^+, q - 1) - H \right] \right. \\
			&\qquad \qquad \left. + \left(1 - e^{-\kappa^+ \delta^+} \Ind_{q > \underline{q}}\right) \E \left[ H(t, x, S + \epsilon^+, q) - H \right] \right\} \\
			&\quad + \lambda^- \sup_{\delta^-} \left\{ e^{-\kappa^- \delta^-} \Ind_{q < \bar{q}} \E \left[ H(t, x - (S - \delta^-), S - \epsilon^-, q + 1) - H \right] \right. \\
			&\qquad \qquad \left. + \left(1 - e^{-\kappa^- \delta^-} \Ind_{q < \bar{q}}\right) \E \left[ H(t, x, S - \epsilon^-, q) - H \right] \right\},
		\end{aligned} \label{eq:10.23}
	\end{equation}
	subject to the terminal condition
	\begin{equation}
		H(T, x, S, q) = x + q (S - \alpha q), \label{eq:10.24}
	\end{equation}
	and where the expectation is over the random variables $\epsilon^\pm$ (not over $x$, $S$, or $q$) and $\Ind$ is the indicator function.
	...
	\begin{equation}
		H(t, x, S, q) = x + q S + h(t, q), \label{eq:10.25}
	\end{equation}
	and substituting it into the DPE we obtain
	\begin{equation}
		\begin{aligned}
			\phi q^2 &= \partial_t h + \lambda^+ \sup_{\delta^+} \left\{ e^{-\kappa^+ \delta^+} \left( \delta^+ - \bar{\epsilon}^+ + h_{q-1} - h_q \right) \right\} \Ind_{q > \underline{q}} \\
			&\quad + \lambda^- \sup_{\delta^-} \left\{ e^{-\kappa^- \delta^-} \left( \delta^- - \bar{\epsilon}^- + h_{q+1} - h_q \right) \right\} \Ind_{q < \bar{q}} \\
			&\quad + (\bar{\epsilon}^+ \lambda^+ - \bar{\epsilon}^- \lambda^-) q,
		\end{aligned} \label{eq:10.26}
	\end{equation}
	subject to $h(T, q) = -\alpha q^2$ which allows us to solve for the optimal controls in feedback form:
	\begin{subequations}
		\begin{align}
			\delta^{+, *}(t, q) &= \frac{1}{\kappa^+} + \bar{\epsilon}^+ - h(t, q-1) + h(t, q), \quad q \neq \underline{q}, \label{eq:10.27a} \\
			\delta^{-, *}(t, q) &= \frac{1}{\kappa^-} + \bar{\epsilon}^- - h(t, q+1) + h(t, q), \quad q \neq \bar{q}. \label{eq:10.27b}
		\end{align}
	\end{subequations}
	
	The interpretation of the optimal controls in feedback form is very similar to what was discussed above. The main difference is that here MOs impact the midprice and this affects the optimal controls in two ways: one is explicitly shown in (10.27) in the form of $\bar{\epsilon}^\pm$, and the other is encoded in the solution of $h(t, q)$.
	It is clear that the MM incorporates the impact of MOs by including the expectation of the jump in prices, conditional on an MO arriving, by posting LOs which are $\epsilon^\perp = \E[\epsilon^\pm]$ further away from the midprice. In this way the MM trader recovers, on average, the losses she incurs due to adverse selection.
	
	Moreover, note that the effects of the jumps in the midprice also feed into the solution of $h(t, q)$ because the optimal strategy must take into account the future arrival of MOs, as these move the prices. Thus, it is important to note that the optimal controls derived here are not the controls as given in (10.8) plus the recovery of the average losses $\epsilon^\pm$ adverse selection costs. This becomes clear when looking at the solution of the DPE which we discuss in the next subsection.
	...
	\bm{\omega}(t) = [\omega(t, \bar{q}), \omega(t, \bar{q}-1), \dots, \omega(t, \underline{q})]'.
	\]
	Furthermore, let $\mathbf{A}$ denote the $(\bar{q} - \underline{q} + 1)$-square matrix whose rows are labelled from $\bar{q}$ to $\underline{q}$ and whose entries are given by
	\begin{equation}
		\mathbf{A}_{i,q} = \begin{cases}
			q \kappa (\bar{\epsilon}^+ \lambda^+ - \bar{\epsilon}^- \lambda^-) - \phi \kappa q^2, & i = q, \\
			\tilde{\lambda}^+, & i = q - 1, \\
			\tilde{\lambda}^-, & i = q + 1, \\
			0, & \text{otherwise},
		\end{cases} \label{eq:10.28}
	\end{equation}
	where $\tilde{\lambda}^\pm = \lambda^\pm e^{-1-\kappa \bar{\epsilon}^\pm}$. Then, on substituting $h$ in terms of $\omega$ in (10.26), we find that
	\[
	\partial_t \bm{\omega}(t) + \mathbf{A} \bm{\omega}(t) = \mathbf{0}.
	\]
	This matrix ODE can be easily solved to find
	\begin{equation}
		\boxed{\bm{\omega}(t) = e^{\mathbf{A}(T-t)} \mathbf{z},} \label{eq:10.29}
	\end{equation}
	where $\mathbf{z}$ is a $(\bar{q} - \underline{q} + 1)$-dim vector where each component is $z_j = e^{-\alpha \kappa j^2}$, $j = \bar{q}, \dots, \underline{q}$. Note that this solution is similar to the one derived above in Section 10.2 but here we have the impact of the MOs on the midprice dynamics.
	
	As a direct consequence of assuming that the shape of the LOB is symmetric, as well as assuming that the rate and impact on midprice of arrival of MOs is the same ($\kappa^\pm = \kappa$, $\lambda^\pm = \lambda$, $\bar{\epsilon}^\pm = \varepsilon$), the MM's optimal depths on the buy side with $q$ shares equals the optimal depth on the sell side with $-q$ shares, $\delta^{\pm *}(t, q) = \delta^{\mp *}(t, -q)$.
	
	\textbf{The Behaviour of the Strategy}
	
	In this section we illustrate several aspects of the behaviour of the optimal strategy as a function of $q$, $\phi$, $\alpha$, $\bar{q}$, and $-\underline{q}$. In Figure 10.8 we show the optimal sell and buy depths when $T = 30$, $\lambda^\pm = 1$, $\kappa^\pm = 100$, $\bar{q} = -q = 3$, $\phi = 0.02$, $\alpha = 0.0001$, $\sigma = 0.01$, $S_0 = 100$, and $\epsilon = 0.005$. The figure shows that when $q = 0$ the optimal buy and sell depths are the same, but when the inventory is $q \neq 0$ the optimal depths are asymmetric (sell depth is different from buy depth) and the fill rates are tilted to induce mean reversion in inventories. For instance, if $q = 2$ the optimal sell depth is lower than the optimal buy depth so that it is more likely for the strategy to sell one unit of the asset than to acquire a unit.
	
	The top panel in Figure 10.9 illustrates further how the fill rates are tilted to induce mean reversion to the optimal level of inventory for different levels of the running inventory parameter $\phi = \{0.2, 0.1, 0.05\}$. The other model parameters are: $\lambda^+ = 2$, $\lambda^- = 1$, $\varepsilon = 0.005$, $\kappa^\pm = 100$, $\bar{q} = -q = 10$, $\alpha = 0.0001$, $\sigma = 0.01$, $S_0 = 100$. The two bottom panels show that the optimal level of inventory for this choice of parameters is to hold a positive amount of shares – this optimal point is located where the inventory drift is zero because this is the level at which the strategy `pulls' inventories.
	
	We discuss in detail the trajectory of inventories when $\phi = 0.05$ which is depicted by the red circles. For example, if $\phi = 0.05$ and the current level of inventory is $q = 4$, the strategy posts symmetrically so that the buy LO is closer to the midprice than the sell LO. In this way it is more likely for inventory to increase (positive inventory drift). Similarly, if the current level of inventory is $q = 6$ the optimal strategy is to post sell LOs closer to the midprice than the buy LOs so that it is more likely that inventory will be reduced (negative inventory drift). The optimal level of inventory is when $q = 5$ where LOs are symmetrically posted around the midprice (buy and sell fill rates are the same). If we follow the same line of reasoning, we see that for $\phi = 0.1$, depicted by green circles, the optimal level of inventory is between 2 and 3 units of the asset; and for $\phi = 0.2$, depicted by blue circles, the optimal level of inventory is approximately 1 unit of the asset.
	...
	\subsection{Short-Term-Alpha and Adverse Selection} \label{sec:10.4.2}
	
	In this section we assume that the midprice of the asset follows
	\begin{equation}
		dS_t = (v + \alpha_t) \, dt + \sigma \, dW_t, \label{eq:10.30}
	\end{equation}
	where the drift is given by a long-term component $v$ and by a short-term component $\alpha_t$ which is a predictable zero-mean reverting process. Here the long- and short-term components are important when devising market making strategies. For example, if the agent is an MM who trades at time scales where she does not `see' the short-term component $\alpha_t$, then her strategy will not only be sub-optimal, but will lose money to better informed traders – traders who are better informed will pick-off the LOs posted by the less informed MM. On the other hand, if the MM has the ability to observe $\alpha_t$ then she will ensure that on average her strategy does not lose money to other traders, and uses this knowledge to execute more speculative trades when $\alpha_t$ is different from zero as we shall show below.
	...
	\begin{equation}
		d\alpha_t = -\zeta \alpha_t \, dt + \eta \, dW^\alpha_t + \epsilon^\alpha_{1+M^+_{t-}} \, dM^+_t - \epsilon^\alpha_{1+M^-_{t-}} \, dM^-_t, \label{eq:10.31}
	\end{equation}
	where $\{\epsilon^\alpha_1, \epsilon^\alpha_2, \dots\}$ are i.i.d. random variables (independent of all processes) representing the size of the sell/buy MO impact on the drift of the midprice. Moreover, $W^\alpha_t$ denotes a Brownian motion independent of all other processes, $\zeta$, $\eta$ are positive constants, and the MOs arrive at an independent constant rate of $\lambda^\pm$.
	
	Now we pose the market making problem where the MM posts only at-the-touch, as we did in Section 10.2.2. However, here the agent's strategy accounts for the influence of short-term-alpha. To this end, let $\ell^\pm_t \in \{0, 1\}$ denote whether she is posted on the sell side (+) or buy side (-) of the LOB. Her performance criteria is as usual
	\[
	H^\ell(t, x, S, \alpha, q) = \E_{t,x,S,\alpha,q} \left[ X^\ell_T + Q^\ell_T \left( S_T - (\tfrac{\Delta}{2} + \varphi Q^\ell_T) \right) - \phi \int_t^T (Q^\ell_u)^2 \, du \right],
	\]
	and her cash process $X^\ell_t$ satisfies the SDE
	\[
	dX^\ell_t = (S_t + \tfrac{\Delta}{2}) \, dN^{+, \ell}_t - (S_t - \tfrac{\Delta}{2}) \, dN^{-, \ell}_t,
	\]
	where $N^{\pm, \ell}_t$ denote the counting processes for her filled LOs. We further assume that, if she is posted in the LOB, when a matching MO arrives her LO is filled with probability one. In this case, $N^{\pm, \ell}_t$ are controlled doubly stochastic Poisson processes with intensity $\ell^\pm_t \lambda^\pm$. (In Exercise E.10.2 we ask the reader to generalise the problem to account for a fill probability less than 1.)
	
	As before, the set $\mathcal{A}$ of admissible strategies are $\mathcal{F}$-predictable such that the agent is not posted on the buy (sell) side if her inventory is equal to the upper (lower) inventory constraints $\bar{q}$ ($q$) and her value function is denoted by
	\[
	...
	\begin{align*}
		0 &= \left( \partial_t + \alpha \partial_S + \tfrac{1}{2}\sigma^2 \partial_{SS} - \zeta \alpha \partial_\alpha + \tfrac{1}{2}\eta^2 \partial_{\alpha \alpha} \right) H - \phi q^2 \\
		&\quad + \lambda^+ \max_{\ell^+ \in \{0,1\}} \left\{ \Ind_{q > \underline{q}} \E \left[ H\left(t, x + (S + \tfrac{\Delta}{2})\ell^+, S, \alpha + \epsilon^+, q - \ell^+\right) - H \right] \right. \\
		&\qquad \qquad \qquad + \left. \left(1 - \ell^+ \Ind_{q > \underline{q}}\right) \E \left[ H(t, x, S, \alpha + \epsilon^+, q) - H \right] \right\} \\
		&\quad + \lambda^- \max_{\ell^- \in \{0,1\}} \left\{ \Ind_{q < \bar{q}} \E \left[ H\left(t, x - (S - \tfrac{\Delta}{2})\ell^-, S, \alpha - \epsilon^-, q + \ell^-\right) - H \right] \right. \\
		&\qquad \qquad \qquad + \left. \left(1 - \ell^- \Ind_{q < \bar{q}}\right) \E \left[ H(t, x, S, \alpha - \epsilon^-, q) - H \right] \right\},
	\end{align*}
	subject to the terminal condition
	\[
	H(T, x, S, \alpha, q) = x + q \left(S - (\tfrac{\Delta}{2} + \varphi q)\right).
	\]
	Here, the expectations are over the random jump sizes $\epsilon^\pm$. The various terms in the DPE carry the following interpretations:
	\begin{itemize}
		\item the first line in the DPE represents the drift and diffusive components of the midprice and the short-term-alpha, as well as the alpha's mean-reverting feature,
		\item the maximisation terms represent the agent's control whether to post an LO at-the-touch,
		\item the second line represents the change in value function, if the agent is posted, due to the arrival of an MO which lifts the agent's offer and simultaneously induces a jump in the short-term-alpha,
		\item the third line represents the change in the value function when an MO arrives, but the agent is not posted – in which case only the short-term-alpha jumps,
		\item the fourth and fifth lines are for the other side of the book.
	\end{itemize}
	...
	\begin{align*}
		0 &= \left( \partial_t - \zeta \alpha \partial_\alpha + \tfrac{1}{2}\eta^2 \partial_{\alpha \alpha} \right) h + \alpha q - \phi q^2 \\
		&\quad + \lambda^+ \max_{\ell^+ \in \{0,1\}} \left\{ \Ind_{q > \underline{q}} \E \left[ \ell^+ \tfrac{\Delta}{2} + h(t, \alpha + \epsilon^+, q - \ell^+) - h(t, \alpha + \epsilon^+, q) \right] \right\} \\
		&\quad + \lambda^- \max_{\ell^- \in \{0,1\}} \left\{ \Ind_{q < \bar{q}} \E \left[ \ell^- \tfrac{\Delta}{2} + h(t, \alpha - \epsilon^-, q + \ell^-) - h(t, \alpha - \epsilon^-, q) \right] \right\} \\
		&\quad + \lambda^+ \E \left[ h(t, \alpha + \epsilon^+, q) - h(t, \alpha, q) \right] \\
		&\quad + \lambda^- \E \left[ h(t, \alpha - \epsilon^-, q) - h(t, \alpha, q) \right],
	\end{align*}
	subject to the terminal condition
	\[
	...
	\]
	The term $\alpha q$ which appears in the first line of the above equation is responsible for making the solution to this problem explicitly dependent on $\alpha$. If it were absent, then the optimal postings and $h$ function would be independent of $\alpha$, since the terminal conditions do not depend on $\alpha$ and there would be no source terms in $\alpha$. However, it is precisely this dependence on $\alpha$ which renders the strategy interesting and allows it to adapt to the adverse selection induced by the arrival of order flow. Finally, the expectation operator $\E$ is with respect to the random jump size $\epsilon$.
	
	The form of the optimising terms allows us to characterise the optimal postings in a compact form. When $\ell = 0$ both terms that are being maximised are zero, hence, the optimal postings of the agent can be characterised succinctly as
	\begin{equation}
		\boxed{
			\begin{aligned}
				\ell^{+, *}(t, q) &= \Ind_{\left\{ \frac{\Delta}{2} + \E[h(t, \alpha + \epsilon^+, q-1) - h(t, \alpha + \epsilon^+, q)] > 0 \right\} \cap \{q > \underline{q}\}}, \\
				\ell^{-, *}(t, q) &= \Ind_{\left\{ \frac{\Delta}{2} + \E[h(t, \alpha - \epsilon^-, q+1) - h(t, \alpha - \epsilon^-, q)] > 0 \right\} \cap \{q < \bar{q}\}}.
			\end{aligned}
		} \label{eq:10.32}
	\end{equation}
	These postings are the analog of the optimal postings in (10.17) from subsection 10.2.2 where we investigated how the agent trades when posting only at-the-touch. The key difference here is that the agent knows that when an MO arrives, $\alpha$ jumps up/down and therefore she compares the expected change in the value functions evaluated at $\alpha \pm \epsilon^\pm$, rather than at $\alpha$, with the half-spread.
	
	\textbf{Features of the Strategy}
	...
	\begin{description}
		\item[E.10.1] Consider the framework developed in subsection 10.2.2, where the MM posts only at-the-touch, but assume that when an MO arrives, and the agent is posted on the matching side of the LOB, her order is filled with probability $\rho < 1$. Derive the DPE and compute the optimal strategy in feedback form. Also, implement the resulting non linear coupled system of ODEs and show how the strategy is altered by the fill probability.
		\item[E.10.2] Consider the framework developed in subsection 10.4.2, where the MM is subject to adverse selection from active traders, but assume that when an MO arrives, and the agent is posted on the matching side of the LOB, her order is filled with probability $\rho < 1$. Derive the DPE and compute the optimal strategy in feedback form. Also, implement the resulting non-linear coupled system of ODEs and show how the strategy is altered by the fill probability.
	\end{description}
