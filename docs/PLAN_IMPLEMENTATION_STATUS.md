...
| Phase 5 - parameter/data pipeline | Automated pass for local pipeline | Atomic writers, atomic strategy-facing snapshot copies, schema v4 tests, status locking, process-level estimator locking, deterministic kappa -> epsilon -> raw-lambda updater order, snapshot validation for timestamped windows/fit diagnostics/toxicity diagnostics, `mo_survival_fit` enforcement for HJB lambda (survival-based per-side MO arrival rates; mid-relative survival-fit kappa; per-MO 200ms-horizon epsilon; direct unsmoothed primaries; sigma2 volatility channel), row-timestamp-based collector freshness validation, and no hardcoded symbol fallback when the strategy has no active pair. |
