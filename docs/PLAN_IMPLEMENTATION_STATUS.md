...
- Unit/integration tests: `python -m pytest tests` passed with 110 tests.
- Runtime gate runner:
...
- Enabled dry-run evidence:
  ...
  fields for parameter age, collector age, HJB age, and orderbook age.
...
| Phase 7 - observability/kill switches | Automated pass for local fields | Health, quote decisions, freshness-age fields, source-labeled exchange/Trade/accepted-confirmation open-order counts, mark-to-mid unrealized PnL, fill accounting, delayed fill markouts, and kill-switch tests/log artifacts exist. |
