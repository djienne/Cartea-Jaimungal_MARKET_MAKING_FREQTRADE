...
One fact about the deployment is needed early, because it leaks into the code
later. The strategy runs inside \textbf{Freqtrade}, an open-source trading
framework whose vocabulary is ``entry'' (opening a position) and ``exit''
(closing one) rather than ``bid'' and ``ask''. Freqtrade rests at most one order
per pair, so the two-sided quote this document describes is not posted by one
process: two instances run on separate sub-accounts --- a long \textbf{leg} and a
short leg --- sharing one net inventory over Redis. Every ``we post a bid and an
ask'' below is really ``the two legs between them post a bid and an ask''.
\S\ref{sec:departures} says what that approximation costs.
...
(\code{merge\_asof} with \code{allow\_exact\_matches=False}), and the walk depth is
...
\snip{scripts/estimator_common.py}{756}{785}
\begin{lstlisting}
...
\snip{scripts/get_epsilon.py}{198}{206}
\begin{lstlisting}
    post = pd.merge_asof(
        frame,
...
\snip{scripts/estimator_common.py}{879}{885}
\begin{lstlisting}
...
replace the last valid model. The strategy then re-checks
...
\begin{intuition}
The toxicity number $\kappa\epsb$ is dimensionless, and it is the most
informative single number you can know about an instrument: adverse-selection
cost measured in units of the natural rent $1/\kappa$.

Its meaning is exact rather than loose, and it is not the one you might guess.
At flat inventory the optimal depth is $1/\kappa + \epsb$, so the rate we
actually get filled at is
\[
...
Now notice what does \emph{not} appear: by \eqref{eq:supvalue} the expected
profit per fill is $1/\kappa$ \textbf{however toxic the flow is}, because the
depth simply steps back far enough to cover it. So \textbf{adverse selection does
not make each trade worse --- it makes trades rarer.} At the live CASHCAT value
$\kappa\epsb \approx 0.25$ it costs about $22\%$ of our fills; at the gate value
of $1.5$ it would cost $78\%$.

This is why the strategy gates on $\kappa\epsb$ and not on $\epsb$ alone, and
why the threshold is where it is. The project's own introduction notebook
(\code{docs/market\_making\_introduction.ipynb}) sets out the operating rule the
shipped \code{max\_toxicity} comes from:
...
$\kappa\epsb$ & verdict \\
\midrule
$< 1$        & tolerable --- profitable if latency is decent \\
$1$ to $2$   & competitive and toxic; needs good latency \emph{and} a good model \\
$\ge 1.5$    & \textbf{the shipped gate}: stop quoting \\
$\ge 2$      & negative PnL close to certain \\
\bottomrule
\end{tabular}
\end{center}

\noindent The argument behind the bands is worth having, because it explains why
$\kappa\epsb$ and not $\epsb$ is the right thing to threshold. $1/\kappa$ is the
half-spread you would quote with no inventory penalty and no adverse selection.
If $\epsb$ is much \emph{larger} than that, then covering adverse selection
pushes your quote far past where anyone would fill it --- so either you never
trade, or the fills you do get are the ones that reached you because you were
\textbf{slow}, which is adverse selection again by another route. Above
$\kappa\epsb \approx 2$ that trap is inescapable; between $1$ and $2$ it is a
pure infrastructure race that most participants lose; below $1$ the flow is
mostly uninformed and the model's arithmetic works as written.

The gate sits at $1.5$ rather than at break-even because the middle band is a
genuine regime: the fill rate is badly damaged, but the trade is still worth
making if the rest of the stack is fast enough. Ours is not, which is
\S\ref{sec:departures} item~8.

Live CASHCAT sits near $0.25$, comfortably in the benign band --- worth
remembering when reading \S\ref{sec:phisweep}, because it means toxicity is
\textbf{not} the reason that sweep loses money.
\end{intuition}

% ===========================================================================
\subsection{What the three parameters say about the instrument}

Before any of them reach a solver, $\kappa$, $\lambda$ and $\epsb$ already answer
a question worth asking first: \emph{is this instrument worth quoting at all?}
The framework earns its keep where
...
\noindent In one line: \textbf{a lot of behavioural noise and little structural
efficiency}. A busy, shallow, uninformed market. The opposite --- a thin tape of
well-informed flow in a deep book --- is where the model is most correct and least
useful, because it will tell you, accurately, to quote almost nothing.
...
\snip{scripts/mm_core.py}{413}{422}
\begin{lstlisting}
    if kappa_avg > 0:
...
\snip{scripts/mm_core.py}{464}{475}
\begin{lstlisting}
    n = len(axis)
...
\snip{scripts/mm_core.py}{492}{498}
\begin{lstlisting}
...
\snip{scripts/mm_core.py}{271}{285}
\begin{lstlisting}
...
\begin{itemize}
  ...
  \item \textbf{The floor is exactly one maker fee, and that is not a
    coincidence.} \code{min\_half\_spread\_bps} and \code{maker\_fee\_rate} are
    both $1.5$~bps, so
% CHECK: floor_equals_fee_cushion = True
    the floor and the fee cushion are the same number to the last bit. The floor
    is therefore not a risk limit --- it is the rule ``never quote inside your own
    fee''.
\end{itemize}

\begin{trap}{How often the model is actually overruled}
Work out where that floor binds and the answer is larger than it looks. Because
floor $=$ fee cushion, $\delta_{\text{total}} = \max(\delta_{\text{model}} +
...
\snip{scripts/hyperliquid_alo_executor.py}{76}{93}
\begin{lstlisting}
...
\snip{scripts/mm_core.py}{314}{325}
\begin{lstlisting}
...
This is also the reason market making is a latency business at all. The race for
speed is not a fashion: it is driven by adverse selection specifically, because
the only two defences are to widen (which \S\ref{sec:estimation} shows has a hard
ceiling --- widen far enough and nobody fills you) or to react faster than the
informed flow. A stack that cannot do the second is left doing the first, and the
sweep in \S\ref{sec:phisweep} is what that looks like when measured: quoting less
and less until it barely trades.
\end{intuition}
...
\code{scripts/estimator\_common.py} & window loading, $\kappa$, $\sigma^2$ \\
...
\code{docs/market\_making\_introduction.ipynb} in this repository is a
step-by-step course on the same model (in French) with an interactive dashboard
that simulates a market and lets you move $\kappa$, $\lambda$, $\epsb$, $\phi$
and $\alpha$ and watch the depths respond. It is the fastest way to build
intuition for the surface of Figure~\ref{fig:skew} before reading any code, and
the toxicity bands in \S\ref{sec:estimation} come from it. Its companion video
(FrenchQuant, \emph{ufNdvd8QCUI} on YouTube) walks through the same material
and is where the reasoning behind those bands is set out.
