...
with reproducible worked examples\par}
...
The worked example uses the pinned startup fit from the clean 2026-09-19
\textbf{CASHCAT} grid, a low-priced token
...
below are executable worked examples, compact enough to explain line by line.
The production trader, dry-run grid and backtest share one Rust model and
execution path; Python is not required to execute or validate those paths.
...
Book equations retain their (10.x) numbers; project equations use D1, D2, and
so on. Source listings are quoted verbatim and checked by
\code{scripts/verify\_spread\_doc.py}. Amber boxes identify implementation
choices outside the book, collected once in \S\ref{sec:departures}.
...
fill rate falls by a factor of $e \approx 2.72$. In the pinned 2026-09-19
CASHCAT snapshot $\kappa^{+} \approx 10{,}754$~USDC$^{-1}$, so
$1/\kappa^{+} \approx 9.30\times10^{-5}$~USDC. That is $5.06$~\textbf{bps}
of a $0.18386$ mid --- a
...
stores them in USDC. Push the ask 5.06~bps
...
\addplot[NavyLink, very thick, domain=0:4e-4, samples=120]
  {0.2185068182684562*exp(-10753.936778090696*x)};
\addlegendentry{$\lambda^{+}e^{-\kappa^{+}\delta}$ (ask)}
\addplot[BoxTrap, thick, dashed, domain=0:4e-4, samples=120]
  {0.2675909514676402*exp(-9227.309868601893*x)};
\addlegendentry{$\lambda^{-}e^{-\kappa^{-}\delta}$ (bid)}
\addplot[gray, dotted, thick] coordinates {(9.2989e-5,0) (9.2989e-5,0.0804)};
\node[gray, anchor=west, font=\scriptsize] at (axis cs:9.8e-5,0.092) {$\delta=1/\kappa^{+}$};
\end{axis}
\end{tikzpicture}
\caption{Fill rate against depth at the pinned 2026-09-19 CASHCAT calibration. At
...
Python path     & explicit JSON artifact for offline analysis & schema v5 \\
...
Its VPIN volume scale uses training data only. Initial order sizes can instead
be specified per variant or restored from a historical checkpoint; future
account states are refused. Saved parameter profiles are used without
refitting, and replay does not update the active calibration cache.
Replay revision \code{receipt-flow-v1} orders events by recorded reception time, falling back explicitly to
exchange time where reception is missing. Accepted L2 snapshots refresh the
best prices. A virtual clock applies the grid's freshness, withdrawal and
resume rules even during silent intervals. These are observed-data gaps, not
proof of the grid's own network disconnects. Comparison uses interval P\&L,
fees and maker fills, not lifetime totals.
...
\snip{scripts/mm_core.py}{430}{439}
\begin{lstlisting}
    if kappa_avg > 0:
...
Here \code{drift} defaults to $\lambda^+\epsilon^+-\lambda^-\epsilon^-$;
the API also accepts an explicit drift estimate.
...
\begin{axis}[
  ...
  ymin=-7e-4, ymax=9e-4,
  ytick={-6e-4,-3e-4,0,3e-4,6e-4,9e-4},
  yticklabels={$-6$,$-3$,$0$,$3$,$6$,$9$},
]
\addplot[NavyLink, very thick, mark=*, mark size=1.3] coordinates {
  (-5,8.66673232e-4) (-4,8.27016385e-4) (-3,7.78434021e-4) (-2,7.15700147e-4)
  (-1,6.27001500e-4) (0,4.74034255e-4) (1,-2.15276308e-4) (2,-3.42976422e-4)
  (3,-4.17969430e-4) (4,-4.71260669e-4) (5,-5.12631968e-4) (6,-5.46453413e-4)};
\addlegendentry{$\delta^{+,*}$ (ask)}
\addplot[BoxTrap, very thick, mark=square*, mark size=1.2, dashed] coordinates {
  (-6,-5.97171730e-4) (-5,-5.57514883e-4) (-4,-5.08932520e-4) (-3,-4.46198646e-4)
  (-2,-3.57499999e-4) (-1,-2.04532754e-4) (0,4.84777809e-4) (1,6.12477923e-4)
  (2,6.87470931e-4) (3,7.40762170e-4) (4,7.82133469e-4) (5,8.15954914e-4)};
\addlegendentry{$\delta^{-,*}$ (bid)}
\addplot[gray, thin, domain=-6.5:6.5] {0};
\end{axis}
\end{tikzpicture}
\caption{The solved control at the pinned 2026-09-19 CASHCAT calibration, $\tau = 130.8$~s.
...
\]
which for the grid's 297.88~USDC, 74\% utilisation, 2$\times$ leverage,
$\bar q = 6$ and $S = 0.18386$ gives $u = 399$~CASHCAT after venue rounding.
A full $\pm6$ band is about 440.16~USDC of notional.
...
\snip{scripts/mm_core.py}{590}{601}
\begin{lstlisting}
    n = len(axis)
...
\snip{scripts/mm_core.py}{618}{624}
\begin{lstlisting}
...
The pinned baseline example uses $\phi\kappa T = 300$ and
$\alpha\kappa = 0.05$: a ratio of $6000$ to $1$. Fixed-fit sweep candidates use
$3000$, making the dominance stronger. The running penalty is what
...
\snip{scripts/mm_core.py}{288}{302}
\begin{lstlisting}
...
Five design choices are encoded here:
...
% CHECK: ask_zero_crossing_q = 0.688
$q = 0.688$. So at this $\tau$, for every inventory above about $0.69$ units ---
almost nine-tenths of the positive band --- the ask is pinned at $1.5$~bps and carries
\textbf{no inventory information at all}, and the bid mirrors it below about $-0.70$.
The skew that \S\ref{sec:model} spends fifteen pages deriving reaches the market,
on the reducing side, only over a narrow band around flat; outside it one side
...
\begin{intuition}[breakable=false]
Figure~\ref{fig:skew} shows the model asking for \emph{negative} ask depths once
$q \ge 1$. A negative depth means ``cross the mid to get out''. That is a
...
\snip{scripts/mm_core.py}{331}{342}
\begin{lstlisting}
...
Everything below is produced by the executable Python worked-example pricing
path, and every tagged number is re-checked by
\code{scripts/verify\_spread\_doc.py}. This checks the example, not the Rust execution engine.

\subsection{Inputs}

The CASHCAT startup fit generated at 2026-09-19 22:46:49Z for clean grid run
\code{run-1789857749175}. Its exchange-time window is
20:46:21.382--22:46:21.382Z: 118.10 observed minutes after one 113.126~s
outage, with 17,819 mid updates and 9,049 trade prints. These market inputs are
recorded measurements; the fractional inventory below is deliberately chosen to
show interpolation.

\begin{center}\small
\begin{tabular}{@{}llll@{}}
\toprule
$\kappa^{+}$ & $10{,}753.937$ & $\lambda^{+}$ & $0.218507$ \\
$\kappa^{-}$ & $9{,}227.310$  & $\lambda^{-}$ & $0.267591$ \\
$\epsb^{+}$  & $2.5247\times10^{-5}$ & $\sigma^{2}$ & $4.9276\times10^{-9}$ \\
$\epsb^{-}$  & $4.2891\times10^{-5}$ & mid $S$ & $0.183860$ \\
\midrule
$T$ & 150 s & unit size $u$ & $399$ CASHCAT \\
$\bar{q}$ & 6 & tick & $10^{-5}$ \\
$(\phi\kappa T)_{\text{target}}$ & 300 & $(\alpha\kappa)_{\text{target}}$ & 0.05 \\
\bottomrule
\end{tabular}
\end{center}

The survival fits have $R^2=0.971/0.992$ on 1,547/1,946 market orders. Their
dimensionless adverse-selection diagnostics are
% CHECK: toxicity_plus = 0.2715
$\kappa^{+}\epsb^{+}=0.2715$ and
% CHECK: toxicity_minus = 0.3958
$\kappa^{-}\epsb^{-}=0.3958$, both below the configured 1.5 rejection ceiling.

Inventory is \textbf{957.6 CASHCAT}, deliberately not a whole number of units --
...
\midrule
% CHECK: kappa_avg = 9990.6233
$\bar{\kappa}$ & $\tfrac12(\kappa^{+}+\kappa^{-})$ & $9990.6233$ \\
% CHECK: phi_kappa_relative = 2.0019e-04
$\phi_{\kappa\text{-rel}}$ & $300/(\bar{\kappa}T)$ & $2.0019\times10^{-4}$ \\
% CHECK: vol_delta = 9.8305e-08
volatility term & $\gamma\sigma^{2}u$ & $9.8305\times10^{-8}$ \\
% CHECK: phi_effective = 2.0029e-04
$\phi$ & sum of the two & $2.0029\times10^{-4}$ \\
% CHECK: alpha_effective = 5.0047e-06
$\alpha$ & $0.05/\bar{\kappa}$ & $5.0047\times10^{-6}$ \\
% CHECK: phi_source = kappa_relative+sigma2
\code{phi\_source} & --- & \code{kappa\_relative+sigma2} \\
% CHECK: n_steps = 76800
\code{n\_steps} & $\lceil T/0.001953125\rceil$ & $76{,}800$ \\
% CHECK: dt = 0.001953125
$dt$ & $T/\text{\code{n\_steps}}$ & $0.001953125$ s \\
\bottomrule
\end{tabular}
\end{center}

The volatility channel contributes about 0.05\% of $\phi$ here, lifting the
dimensionless product from the configured $300$ to
% CHECK: phi_kappa_t_dimensionless = 300.1473
$\phi\bar{\kappa}T = 300.1473$ -- below the cap of 450.
...
\midrule
% CHECK: rent_bps = 5.0576
$1/\kappa^{+}$ & rent & $+9.2989\times10^{-5}$ & $+5.06$ \\
% CHECK: adverse_bps = 1.3732
$\epsb^{+}$ & adverse-selection cover & $+2.5247\times10^{-5}$ & $+1.37$ \\
% CHECK: h_diff_q2 = 4.6121e-04
% CHECK: inventory_bps = -25.0850
$-\bigl(h(\tau,1)-h(\tau,2)\bigr)$ & inventory pressure & $-4.6121\times10^{-4}$ & $-25.08$ \\
\midrule
% CHECK: delta_plus_q2_bps = -18.6542
$\delta^{+,*}(\tau,2)$ & & $-3.4298\times10^{-4}$ & $-18.65$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent The inventory term is more than three times the other two combined, and it
...
and it is willing to pay $18.7$~bps through the mid to get there. The first two
terms --- the entire content of \S\ref{sec:model} up to \eqref{eq:1027} --- are
worth $6.43$~bps between them and are simply overwhelmed. Everything that makes
...
\midrule
% CHECK: delta_plus_q2 = -3.4298e-04
% CHECK: delta_plus_q3 = -4.1797e-04
% CHECK: ask_delta_model = -3.7297e-04
$\delta^{+,*}$ (ask) & $-3.4298\times10^{-4}$ & $-4.1797\times10^{-4}$ & $-3.7297\times10^{-4}$ \\
% CHECK: delta_minus_q2 = 6.8747e-04
% CHECK: delta_minus_q3 = 7.4076e-04
% CHECK: bid_delta_model = 7.0879e-04
$\delta^{-,*}$ (bid) & $6.8747\times10^{-4}$ & $7.4076\times10^{-4}$ & $7.0879\times10^{-4}$ \\
\bottomrule
\end{tabular}
\end{center}

Check the ask by hand: $-3.4298\times10^{-4} + 0.4\times(-4.1797 + 3.4298)\times
10^{-4} = -3.7297\times10^{-4}$. Note it is \textbf{negative}: holding 2.4 units
...
% CHECK: bid_delta_model = 7.0879e-04
% CHECK: ask_delta_model = -3.7297e-04
model depth $\delta_{\text{model}}$ & $7.0879\times10^{-4}$ & $-3.7297\times10^{-4}$ \\
% CHECK: bid_fee_cushion = 2.7579e-05
% CHECK: ask_fee_cushion = 2.7579e-05
$+$ fee cushion & $2.7579\times10^{-5}$ & $2.7579\times10^{-5}$ \\
% CHECK: bid_delta_pre_clamp = 7.3637e-04
% CHECK: ask_delta_pre_clamp = -3.4539e-04
$=$ pre-clamp & $7.3637\times10^{-4}$ & $-3.4539\times10^{-4}$ \\
\midrule
% CHECK: floor_price = 2.7579e-05
floor ($1.5$ bps) & \multicolumn{2}{c}{$2.7579\times10^{-5}$} \\
% CHECK: cap_price = 1.4709e-03
cap ($80$ bps) & \multicolumn{2}{c}{$1.4709\times10^{-3}$} \\
% CHECK: bid_clamped = None
% CHECK: ask_clamped = floor
clamped? & no & \textbf{yes, at the floor} \\
\midrule
% CHECK: bid_delta_total = 7.3637e-04
% CHECK: ask_delta_total = 2.7579e-05
final $\delta_{\text{total}}$ & $7.3637\times10^{-4}$ & $2.7579\times10^{-5}$ \\
% CHECK: bid_bps = 40.0504
% CHECK: ask_bps = 1.5000
in bps of mid & $40.05$ & $1.50$ \\
\midrule
% CHECK: bid_raw_price = 0.18312363
% CHECK: ask_raw_price = 0.18388758
raw price ($S \mp \delta$) & $0.18312363$ & $0.18388758$ \\
% CHECK: bid_price = 0.18312
% CHECK: ask_price = 0.18389
rounded to tick & $0.18312$ (down) & $0.18389$ (up) \\
\bottomrule
\end{tabular}
\end{center}
...
% CHECK: quoted_spread_bps = 41.8797
The final quote is wildly asymmetric: the ask sits $1.50$ bps from the mid and
the bid $40.05$ bps away. Those sum to $41.55$ bps, while the quoted spread
measured between the two \emph{rounded} prices is $41.88$ bps --- the extra
$0.33$ bps is tick rounding pushing both prices away from the touch.
...
% CHECK: ask_fill_rate_per_sec = 0.1624
% CHECK: ask_seconds_per_fill = 6.2
$\lambda^{+}e^{-\kappa^{+}\delta} = 0.1624$~s$^{-1}$ --- a modelled fill every
$6.2$ seconds. The bid, $40.05$~bps out, runs at $0.000300$~s$^{-1}$: a modelled
fill every $55.6$~minutes, against an episode horizon of $150$~seconds.
...
\midrule
% CHECK: q0p0_bid_bps = 27.87
% CHECK: q0p0_ask_bps = 27.28
$0.0$ & $27.87$ & $27.28$ & no & near-symmetric; the model as the book intends it \\
% CHECK: q0p4_bid_bps = 30.64
% CHECK: q0p4_ask_bps = 12.29
$0.4$ & $30.64$ & $12.29$ & no & mild skew, both sides live \\
% CHECK: q2p4_bid_bps = 40.05
% CHECK: q2p4_ask_bps = 1.50
$2.4$ & $40.05$ & $1.50$ & ask & the worked example \\
...
% CHECK: bid_outside_calibrated = True
    At $7.36\times10^{-4}$ it sits beyond the 95th percentile of observed
    market-order walk depths ($3.05\times10^{-4}$). Equation~\eqref{eq:fillrate}
...
\begin{center}\small
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{6.2cm}>{\raggedright\arraybackslash}p{8.0cm}@{}}
\toprule
...
\nolinkurl{rust_live/src/backtest.rs} & Rust event replay using the shared paper execution path \\
