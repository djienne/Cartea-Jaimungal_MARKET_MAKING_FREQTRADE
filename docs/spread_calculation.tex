...
\snip{scripts/get_kappa.py}{270}{278}
\begin{lstlisting}
...
\snip{scripts/mm_core.py}{457}{466}
\begin{lstlisting}
    if kappa_avg > 0:
...
current Rust profile uses 200 (ceiling 300), so the table below is historical
...
\snip{scripts/mm_core.py}{617}{628}
\begin{lstlisting}
    n = len(axis)
...
\snip{scripts/mm_core.py}{645}{651}
\begin{lstlisting}
...
$\alpha\kappa = 0.05$: a ratio of $200$ to $1$. The current Rust profile's 200
...
evidence. It affects episodic quotes and remains unswept; treat it as untuned.
\end{departure}
...
\snip{scripts/mm_core.py}{315}{329}
\begin{lstlisting}
...
\snip{scripts/mm_core.py}{358}{369}
\begin{lstlisting}
...
scientifically relevant, but not automatically profitable: the 161.95-hour
replay reversed the earlier latency ordering, and the 185-hour width--cadence
matrix was non-monotone. The defensible conclusions come from measured markouts,
independent frozen windows, and explicit scenario assumptions, not from a generic
``faster is always better'' rule.
\end{intuition}
