...
Everything in this document is calibrated on \textbf{CASHCAT}, a low-priced token
...
\bottomrule
\end{tabular}
\caption{The current runtime cadence and data boundary. Python JSON snapshots are
analysis artifacts; the Rust trader calibrates directly from Parquet.}
\end{table}

A cycle that fails its own quality checks is rejected, so one bad window cannot
...
HJB, at flat inventory and before fees or clamps, the optimal depth is
$1/\kappa + \epsb$, so the fitted fill rate there is
\[
...
$\phi\kappa T = 10$ was the Python replay default used by this dated sweep. The
current Rust profile uses 300 (ceiling 450), so the table below is historical
evidence about shape, not a statement of the current configuration.
...
\midrule
 ...
 10.00 & $-95.95$  & 1405 & $-0.068$ & \textbf{historical default} \\
...
\node[BoxDeparture, anchor=south west, font=\scriptsize, rotate=90]
  at (axis cs:10.7,-148) {historical default};
...
\end{semilogxaxis}
\end{tikzpicture}
\caption{The same data. Past $\phi\kappa T \approx 1$ the loss shrinks
steadily --- with one reversal, $20 \to 50$, well inside the run-to-run noise
of a single tape --- and never turns back up within a factor of 400.}
\label{fig:phisweep}
\end{figure}
...
penalty at all --- it is evidence that on this instrument and tape, at this
simulated latency, the marginal maker fill has negative expected value. No
tested setting of $\phi$ fixes that result.
\end{trap}
...
check in the Python test suite.
...
\begin{departure}{Departure from the book}
The Python field \code{hjb\_alpha\_kappa} (Rust: \code{alpha\_kappa}) was set to
$0.05$ when only the $t=0$ slice was read, so the value carries no tuning
evidence. Under the current $\phi\kappa T=300$, its effect is confined to the
final 8.75 seconds; a later sweep found bit-identical PnL at 0.05, 0.5, and 5.0.
It remains untuned because this regime cannot resolve it.
\end{departure}
...
what \eqref{eq:1022} means, while a resting quote remains exposed to subsequent
flow until it is filled or cancelled. That makes latency and hold policy
scientifically relevant, but not automatically profitable: multi-day replay
ladders have reordered across tapes, and the 185-hour width--cadence matrix was
non-monotone. The defensible conclusions come from measured markouts, independent
frozen windows, and explicit scenario assumptions, not from a generic ``faster
