...
9.9\times10^{-5}$~USDC. That is $9.3$~\textbf{bps} of a $0.1057$ mid --- a
...
stores them in USDC. Push the ask 9.3~bps
...
\snip{scripts/estimator_common.py}{730}{759}
\begin{lstlisting}
...
\snip{scripts/get_kappa.py}{266}{274}
\begin{lstlisting}
...
\snip{scripts/estimator_common.py}{853}{859}
\begin{lstlisting}
...
% CHECK: ask_zero_crossing_q = 1.107
$q = 1.107$. So at this $\tau$, for every inventory above about $1.1$ units ---
...
One recorded CASHCAT estimator cycle, stamped
...
\midrule
% CHECK: rent_bps = 9.3470
$1/\kappa^{+}$ & rent & $+9.8784\times10^{-5}$ & $+9.35$ \\
% CHECK: adverse_bps = 2.3688
$\epsb^{+}$ & adverse-selection cover & $+2.5035\times10^{-5}$ & $+2.37$ \\
% CHECK: h_diff_q2 = 2.2846e-04
% CHECK: inventory_bps = -21.6170
$-\bigl(h(\tau,1)-h(\tau,2)\bigr)$ & inventory pressure & $-2.2846\times10^{-4}$ & $-21.62$ \\
\midrule
% CHECK: delta_plus_q2_bps = -9.9012
$\delta^{+,*}(\tau,2)$ & & $-1.0464\times10^{-4}$ & $-9.90$ \\
\bottomrule
\end{tabular}
\end{center}
...
and it is willing to pay $9.9$~bps through the mid to get there. The first two
terms --- the entire content of \S\ref{sec:model} up to \eqref{eq:1027} --- are
worth $11.7$~bps between them and are simply overwhelmed. Everything that makes
...
% CHECK: ask_fill_rate_per_sec = 0.0964
% CHECK: ask_seconds_per_fill = 10.4
$\lambda^{+}e^{-\kappa^{+}\delta} = 0.0964$~s$^{-1}$ --- a fill every $10.4$
seconds. The bid, $44.85$~bps out, runs at $0.00137$~s$^{-1}$: a fill every
$12$~minutes, against an episode horizon of $150$~seconds.
...
\midrule
% CHECK: q0p0_bid_bps = 24.58
% CHECK: q0p0_ask_bps = 22.03
$0.0$ & $24.58$ & $22.03$ & no & symmetric; the model as the book intends it \\
% CHECK: q0p4_bid_bps = 29.02
% CHECK: q0p4_ask_bps = 14.29
$0.4$ & $29.02$ & $14.29$ & no & mild skew, both sides live \\
% CHECK: q2p4_bid_bps = 44.85
